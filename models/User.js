...
class User {
  ...
  static async create(userData) {
    try {
      ...
      return new Promise((resolve, reject) => {
        db.run(
          'INSERT INTO users (id, username, password, avatar_path, user_role, status, disk_limit, package_name, stream_limit, expired_at, phone) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
          [userId, userData.username, hashedPassword, userData.avatar_path || null, userData.user_role || 'admin', userData.status || 'active', userData.disk_limit || 0, userData.package_name || 'custom', userData.stream_limit || 0, userData.expired_at || null, userData.phone || null],
          function (err) {
            if (err) {
            ...
            resolve({ id: userId, username: userData.username, user_role: userData.user_role || 'admin', status: userData.status || 'active', disk_limit: userData.disk_limit || 0, package_name: userData.package_name || 'custom', stream_limit: userData.stream_limit || 0, expired_at: userData.expired_at || null, phone: userData.phone || null });
          }
        );
      });
    } catch (error) {
  ...
  static updateProfile(userId, updateData) {
    return new Promise((resolve, reject) => {
      ...
      if (updateData.package_name !== undefined) {
        ...
        values.push(updateData.package_name);
      }

      if (updateData.stream_limit !== undefined) {
        fields.push('stream_limit = ?');
