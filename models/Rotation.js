...
class Rotation {
  ...
  static normalizeRotation(row) {
    if (!row) return row;
    row.is_loop = row.is_loop === 1 || row.is_loop === true;
    row.global_youtube_altered_content = row.global_youtube_altered_content === 1 || row.global_youtube_altered_content === true;
    row.global_youtube_made_for_kids = row.global_youtube_made_for_kids === 1 || row.global_youtube_made_for_kids === true;
    return row;
  }
  ...
  static findById(id) {
    return new Promise((resolve, reject) => {
      db.get('SELECT * FROM stream_rotations WHERE id = ?', [id], (err, row) => {
        ...
        resolve(Rotation.normalizeRotation(row));
      });
    });
  }
    ...
    return new Promise((resolve, reject) => {
      db.all(
        ...
        (err, rows) => {
          ...
          if (rows) {
            rows.forEach(row => Rotation.normalizeRotation(row));
          }
          resolve(rows || []);
        }
      );
  ...
  static update(id, rotationData, userId = null) {
    const fields = [];
    const values = [];

    Object.entries(rotationData).forEach(([key, value]) => {
      if ((key === 'is_loop' || key === 'global_youtube_altered_content' || key === 'global_youtube_made_for_kids') && (typeof value === 'boolean' || value === 'true' || value === 'false')) {
        fields.push(`${key} = ?`);
    ...
    let query = `UPDATE stream_rotations SET ${fields.join(', ')} WHERE id = ?`;
    if (userId) {
      query += ' AND user_id = ?';
      values.push(userId);
    }
    ...
    return new Promise((resolve, reject) => {
      db.run(query, values, function(err) {
        ...
        resolve({ id, ...rotationData, updated: this.changes > 0 });
      });
    });
  }
  ...
  static addItem(itemData) {
    ...
    return new Promise((resolve, reject) => {
      db.run(
        `INSERT INTO rotation_items (id, rotation_id, order_index, video_id, title, description, tags, thumbnail_path, original_thumbnail_path, privacy, category, youtube_monetization, youtube_altered_content, youtube_made_for_kids)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [id, rotation_id, order_index, video_id, title, description, tags, thumbnail_path, original_thumbnail_path, privacy, category, youtube_monetization ? 1 : 0, youtube_altered_content ? 1 : 0, youtube_made_for_kids ? 1 : 0],
        function(err) {
          if (err) {
  ...
  static updateItem(itemId, itemData) {
    ...
    Object.entries(itemData).forEach(([key, value]) => {
      ...
      } else if (key === 'youtube_altered_content' && typeof value === 'boolean') {
        values.push(value ? 1 : 0);
      } else if (key === 'youtube_made_for_kids' && typeof value === 'boolean') {
        values.push(value ? 1 : 0);
      } else {
        values.push(value);
      }
