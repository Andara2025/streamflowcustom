  ...
  return new Promise((resolve, reject) => {
    db.serialize(() => {
      ...
      db.run(`ALTER TABLE rotation_items ADD COLUMN youtube_altered_content INTEGER DEFAULT 0`, (err) => {
        if (err && !err.message.includes('duplicate column name')) {
          console.error('Error adding youtube_altered_content column to rotation_items:', err.message);
        }
      });

      db.run(`ALTER TABLE rotation_items ADD COLUMN youtube_made_for_kids INTEGER DEFAULT 0`, (err) => {
        if (err && !err.message.includes('duplicate column name')) {
          console.error('Error adding youtube_made_for_kids column to rotation_items:', err.message);
        }
      });
      ...
      db.run(`ALTER TABLE stream_rotations ADD COLUMN global_youtube_altered_content INTEGER DEFAULT 0`, (err) => {
        if (err && !err.message.includes('duplicate column name')) {
          console.error('Error adding global_youtube_altered_content column to stream_rotations:', err.message);
        }
      });

      db.run(`ALTER TABLE stream_rotations ADD COLUMN global_youtube_made_for_kids INTEGER DEFAULT 0`, (err) => {
        if (err && !err.message.includes('duplicate column name')) {
          console.error('Error adding global_youtube_made_for_kids column to stream_rotations:', err.message);
        }
      });
      ...
      db.run(`ALTER TABLE youtube_channels ADD COLUMN youtube_client_id TEXT`, (err) => {
        if (err && !err.message.includes('duplicate column name')) {
          console.error('Error adding youtube_client_id column to youtube_channels:', err.message);
        }
      });

      db.run(`ALTER TABLE youtube_channels ADD COLUMN youtube_client_secret TEXT`, (err) => {
        if (err && !err.message.includes('duplicate column name')) {
          console.error('Error adding youtube_client_secret column to youtube_channels:', err.message);
        }
      });
      ...
      db.run(`ALTER TABLE streams ADD COLUMN youtube_altered_content INTEGER DEFAULT 0`, (err) => {
        if (err && !err.message.includes('duplicate column name')) {
          console.error('Error adding youtube_altered_content column to streams:', err.message);
        }
      });

      db.run(`ALTER TABLE streams ADD COLUMN youtube_made_for_kids INTEGER DEFAULT 0`, (err) => {
        if (err && !err.message.includes('duplicate column name')) {
          console.error('Error adding youtube_made_for_kids column to streams:', err.message);
        }
      });
